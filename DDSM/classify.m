...
load('cancer01.mat');
load('benign01.mat');
benign01_features(:,31) = -1*ones(size(benign01_features,1),1);
size(cancer01_features)
size(benign01_features)
roi_features = [cancer01_features;benign01_features];
for i=1:size(roi_features,1)
    if( roi_features(i,31)==0 )
        roi_features(i,31)=-1;
    end
end
rng(9);
...
avgTestConf(1,1)+avgTestConf(2,2)
